...
void settime(char key)
{
    ...
    if(once == 0)
    {
    byte = read_ds1307(0x07);
    byte = byte | 0x80;
    write_ds1307(0x07, byte);

    byte = read_ds1307(0x00);
    scd = ((byte >> 4 )& 0x07);
    scd = scd * 10;
    scd = scd + (byte & 0x0F);

    byte = read_ds1307(0x01);
    mnn = ((byte >> 4 )& 0x07);
    mnn = mnn * 10;
    mnn = mnn + (byte & 0x0F);

    byte = read_ds1307(0x02);
    hrr = ((byte >> 4 )& 0x03);
    hrr = hrr * 10;
    hrr = hrr + (byte & 0x0F);
    ...
    else if (key == 21)
    {
        ...
        write_ds1307(0x00, byte);

        byte = (((mnn / 10) << 4) & 0x70)|((mnn % 10) & 0x0F);
        write_ds1307(0x01, byte);

        byte = (((hrr / 10) << 4) & 0x30)|((hrr % 10) & 0x0F);
        write_ds1307(0x02, byte);
