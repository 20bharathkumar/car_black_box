...
unsigned char index, hr, min, sec, speed;
...
unsigned char uart_getch_with_timeout(unsigned short max_time);
...
void download_log()
{
    ...
    if (e >= 0 && e < 10)
    {
      ...
      sprintf(str,"    %c   %c%c:%c%c:%c%c     %c%c      %c%c  \n\r",
            ...
            read_external_eeprom(p*10 ),
            read_external_eeprom(p*10 + 1),
            read_external_eeprom(p*10 + 2),
            read_external_eeprom(p*10 + 3),
            read_external_eeprom(p*10 + 4),
            read_external_eeprom(p*10 + 5),
            read_external_eeprom(p*10 + 6),
            read_external_eeprom(p*10 + 7),
            read_external_eeprom(p*10 + 8),
            read_external_eeprom(p*10 + 9) );
